...
export default function Login({ onLoginSuccess, onShowRegister }: LoginProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); setError(""); setLoading(true); try { const response = await loginUser(form.email, form.password); onLoginSuccess(response.access_token); } catch (requestError) { setError(getApiErrorMessage(requestError, t("loginFailed"))); } finally { setLoading(false); } }
